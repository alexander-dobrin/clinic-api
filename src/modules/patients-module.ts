...
export const patientsModule = new ContainerModule((bind: interfaces.Bind) => {
    bind<IRoutes>(TYPES.PATIENTS_ROUTES).to(PatientsRoutes).inSingletonScope();
    bind<IHttpController>(TYPES.PATIENTS_CONTROLLER).to(PatientsController).inSingletonScope();
    bind<IPatientsService>(TYPES.PATIENTS_SERVICE).to(PatientsService).inSingletonScope();
    bind<IRepository<PatientModel>>(TYPES.PATIENTS_REPOSITORY).to(PatientsRepository).inSingletonScope();
