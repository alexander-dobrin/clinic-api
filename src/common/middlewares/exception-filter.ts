...
export class ExceptionFilter {
	public catch(err: Error, req: Request, res: Response, next: NextFunction) {
		if (err instanceof InvalidParameterError || err instanceof DuplicateEntityError) {
			res.status(StatusCodeEnum.BAD_REQUEST).json({ error: { message: err.message } });
			return;
		} else if (err instanceof AppointmentConflictError || err instanceof UserConfilctError) {
			res.status(StatusCodeEnum.CONFLICT).json({ error: { message: err.message } });
			return;
		} else if (err instanceof UnableToSortError || err instanceof UnprocessableEntityError) {
