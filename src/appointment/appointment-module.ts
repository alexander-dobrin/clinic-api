...
export const appointmentModule = new ContainerModule((bind: interfaces.Bind) => {
    ...
    bind<IHttpController>(CONTAINER_TYPES.APPOINTMENTS_CONTROLLER).to(AppointmentController).inSingletonScope();
    bind<AppointmentService>(CONTAINER_TYPES.APPOINTMENTS_SERVICE).to(AppointmentService).inSingletonScope();
    bind<IRepository<AppointmentModel>>(CONTAINER_TYPES.APPOINTMENTS_REPOSITORY).to(AppointmentRepository).inSingletonScope();
