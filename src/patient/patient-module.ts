...
export const patientModule = new ContainerModule((bind: interfaces.Bind) => {
    ...
    bind<IHttpController>(CONTAINER_TYPES.PATIENTS_CONTROLLER).to(PatientController).inSingletonScope();
    bind<PatientService>(CONTAINER_TYPES.PATIENTS_SERVICE).to(PatientService).inSingletonScope();
    bind<IRepository<PatientModel>>(CONTAINER_TYPES.PATIENTS_REPOSITORY).to(PatientRepository).inSingletonScope();
